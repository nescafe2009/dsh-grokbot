...
test('efficiency exposes missing usage, request retries and actual model without ranking roles',async()=>{
 const {EfficiencyPanel}=await import(await import('./ctb-path.mjs').then(m=>m.CTB_DIR+'/test-entry.mjs'))
 const host=document.createElement('div');document.body.append(host);const root=createRoot(host)
 try{
  const data={sessions:1,observedCalls:1,firstResponseP50Ms:null,firstResponseP95Ms:null,retryCount:1,scope:'已采样，非账单',calls:[{sessionId:'s',turn:1,step:1,startedAt:100,endedAt:null,observedAt:310000,provider:'zai',model:'glm-5.3',inputReported:null,firstResponseMs:null,elapsedMs:309900,toolMs:0,toolCount:0,retryCount:1,usage:null,attempts:[{outcome:'TIMEOUT',firstResponseMs:null,elapsedMs:300000},{outcome:'pending',firstResponseMs:null,elapsedMs:9000}]}]}
  await act(async()=>root.render(React.createElement(EfficiencyPanel,{data})))
  assert.match(host.textContent,/输入 未知/);assert.match(host.textContent,/TIMEOUT/)
  assert.match(host.textContent,/glm-5.3/);assert.match(host.textContent,/不用于角色能力排名/)
  assert.match(host.textContent,/输出：未知/);assert.match(host.textContent,/尚未记录结束/)
