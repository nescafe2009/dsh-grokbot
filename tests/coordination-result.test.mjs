...
async function run(outcome,{writeFails=false, beforeInput=null, duringRead=null}={}){
 ...
 const c={prepareProjectHandoff:async()=>{},projectCanRun:async()=>true,disposed:false,LONG_TASK_RULES,crewState:{crew:{bots:[{id:'chief'}],conversations:[{id:'g',name:'G'}]}},botState:()=>state,coordinationClaims:new Set(),busyProbes:new Map(),chiefWake:wake,readRoomMsgs:async()=>{if(duringRead)await duringRead(wake);return [{role:'system',text:'member failed'}]},logWake:x=>logs.push(x),chiefProjectContext:async()=>({}),readPlan:async()=>({}),stateDir:'/fixture',chatTurn:async(bot,input)=>{if(beforeInput)await beforeInput(wake);if(typeof input==='function')await input();return outcome},appendRoomMsg:async(id,msg)=>{if(writeFails)throw Error('disk full');messages.push(msg)},ctx:{logger:{}},safeError:String,Date,clearInterval,setInterval}
