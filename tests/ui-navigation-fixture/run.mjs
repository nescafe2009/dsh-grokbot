...
const html = `<!doctype html><html><head><meta charset="utf-8"><title>ui-navigation-fixture</title></head>
<body style="margin:12px"><div id="root"></div><script>${js.replace(/<\/script>/g, '<\\/script>')}</script></body></html>`

const server = createServer(async (req, res) => {
  if (req.url === '/client.js') { res.writeHead(200, {'content-type':'text/javascript','cache-control':'no-store'});res.end(await readFile(process.env.UI_CLIENT_BUNDLE || join(repoRoot,'lib/client.js')));return }
  const asset = /^\/api\/plugins\/grokbot\/assets\/avatars\/([a-z0-9-]+)$/.exec(req.url ?? '')
  if (asset) {
    try { const svg = await readFile(join(repoRoot,'assets-design','avatars',asset[1]+'.svg')); res.writeHead(200,{'content-type':'image/svg+xml'});res.end(svg) }
...
console.log(JSON.stringify({ event: 'ui-navigation-fixture-ready', url: 'http://127.0.0.1:8796/' }))
