...
验收通过只确认本轮交付，不自动归档或移除成员。归档需要当前用户明确提出，不能从幕僚长预先捆绑的“通过后归档”推定授权。同一产品的后续修改沿用原项目团队。completed/archived可通过iterate开启新一轮active迭代，保留旧计划与验收，并快照上轮基线；epoch递增防止旧队列复活，不自动派工。paused/cancelled仍需明确恢复其暂停/取消边界。缺陷走原阶段返工复测，新需求追加实现、测试和最终验收阶段；旧finalDelivery改为false后指定新最终交付。已有专业团队的产品代码修改仍由成员负责，幕僚长组织和核验，不能以小改动绕过团队。

## Codex 委托验收与已有成果复用（2026-09-12）

项目宿主配置 `externalReviewer={kind:'codex',threadId}` 后，最终阶段及原 user 决策关卡交给 Codex 独立验收，chief 技术自检保持原分工。依赖全部通过、没有执行任务的 `finalDelivery` 汇总自动呈现 `awaiting_acceptance / codex`，不需要派零修改汇总任务。普通项目的用户验收分工保持原规则。

`external-reviews` 是持久待审队列，绑定项目、迭代、阶段/依赖版本及当前登记文件的 SHA-256。扫描、重启和重复读取不会产生重复待审项；文件变化、返工或归档使旧请求失效。哈希覆盖登记文件，并不证明所有应该交付的文件都已登记；审核人仍须检查交付范围。队列读取不代表 Codex 已收到推送或用户已读，当前没有后台自动唤醒 Codex 的连接。

宿主操作入口 `POST /external-reviews/:projectId/decision` 要求本机私有审核凭证及匹配的 `threadId`，不注册为幕僚长工具，不在浏览器状态和模型上下文中返回凭证。所有决定在 DSH 宿主进程内执行，与其他项目操作共享锁。验收记录 `actor=codex`；驳回必须指定缺陷阶段、复测人及标准，实际开启返工并撤销受影响下游验收。不得用直接修改运行中生命周期 JSON 代替此入口。

`team_register_delivery` 用于前置返工后仍适用的真实成果：必须附同项目、同迭代、同负责人来源任务及 checkpoint 文件，指定当前 revision/epoch/generation/fingerprint/依赖版本，并说明重新核验依据。只登记待审，不创建 job，也不批准阶段。正在执行、有未完成修复/复测、跨项目来源或旧版本请求均拒绝。已有有效登记时重复派工被拦截；缺陷仍须正式退回。

规则通知使用 chat 请求的 `executionMode=notice`：持久化通知及确定性回执，不创建模型会话、不调用工具。同一 requestId 不能从通知改成执行指令。普通自然语言消息仍按执行请求处理，不能仅凭“请只确认”假定无副作用。

项目快照提供 `executionHistory`（最近200次派发及总数），包含取消、失败、修复、复测的原始起止时间。无 job 的协调会话和历史缺失 token 未被补造；不得用行数、耗时或全账户额度百分比推导单项目 token 节省。
