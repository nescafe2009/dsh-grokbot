...
export function ProjectBoard({conversationId,bots,load,onApproval,onBot}:{conversationId:string;bots:Member[];load:(id:string)=>Promise<BoardData>;onApproval:()=>void;onBot?:(id:string)=>void}):ReactNode {
 ...
 const renderRow=(row:BoardRow,index:number)=>{
  ...
  return <li key={`${row.source}:${row.id}`} data-status={row.status}>
   ...
   <div className="gk-board__item">
    ...
    <div className="gk-board__meta">{onBot?<button type="button" className="gk-board__owner" onClick={()=>onBot(row.botId)} title="查看成员执行记录">{owner} ↗</button>:<span>{owner}</span>}<span className="gk-board__status">{row.status==='done'&&row.source!=='plan'?'执行结束 · 待核实':row.status==='awaiting_acceptance'?(row.reviewMode==='chief'?'等待幕僚长核验':row.reviewMode==='codex'?'等待 Codex 验收':'等待审阅决定'):labels[row.status]||'待核实'}</span></div>
 ...
 return <aside className="gk-board" data-collapsed={collapsed} aria-label="群聊任务列表">
  <header><h2>任务</h2><span className="gk-board__count">{current?`${done} / ${tasks.length}`:'加载中'}</span></header>
  {!collapsed?<>
