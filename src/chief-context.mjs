...
import {workingBoardContext,workingContextSize} from './context-policy.mjs'
import {compactUserDirectiveView,userDirectiveView} from './context-view.mjs'
...
export async function chiefBrief({crew,board,history,dm,selection,projectId,detail='current'}){
 ...
 // Every non-terminal project must remain discoverable. Apply display limits only
 // after lifecycle classification so recent archived rooms cannot evict an older
 // active project from the authority view.
 ...
 for(const c of chosen){
  try{
   ...
   projects.push({lifecycle:state.lifecycle,contextDetail:detail,historyOmitted:state.historyOmitted,planRevision:state.planRevision||0,id:c.id,name:c.name,members:c.memberBotIds.map(id=>({id,name:crew.bots.find(b=>b.id===id)?.name||id})),
    executionHistory:state.executionHistory,executionHistoryTotal:state.executionHistoryTotal,executionHistoryNote:state.executionHistoryNote,evidenceIndex:state.evidenceIndex,
    ...
    tasks:detail==='working'?state.rows:state.rows.slice(0,60),additionalTasks:detail==='working'?0:Math.max(0,state.rows.length-60),
    userInstructions:detail==='working'?directives.items:messages.filter(m=>m.role==='user').slice(-6).map(m=>excerpt(m,3000,'user_message','用户')),
    ...(directives?{instructionHistory:{...directives,items:undefined}}:{}),
 ...
 // Global user constraints can originate in DM. Do not discard them using a
 // relevance guess; omit model/system chatter, but keep user wording intact.
 const dmMessages=await dm()
 // A project session must know that global DM evidence exists, but replaying
 // every full DM instruction on every turn makes long-running groups explode.
 // Keep a bounded catalogue here; query-time retrieval restores exact matches.
 ...
 const recentDm=detail==='working'?dmDirectives.items.map(item=>({...item,conversationId:item.scope,scope:'幕僚长私聊；逐条核对适用项目，不是当前项目的新授权'})):dmMessages.slice(-8).map(m=>excerpt(m,1800,m.role==='user'?'user_message':m.role==='bot'?'chief_previous_reply_unverified':'system_event',m.role==='user'?'用户':m.role==='bot'?'幕僚长':'系统'))
 const visibleArchived=projectId?archivedProjects:archivedProjects.slice(-12)
 const result={archivedProjects:visibleArchived,archivedProjectCount:archivedProjects.length,currentModel:selection,projectCount:projects.length,projectIndex:projects.map(c=>({id:c.id,name:c.name,lifecycle:c.lifecycle?{status:c.lifecycle.status,revision:c.lifecycle.revision,epoch:c.lifecycle.epoch,iteration:c.lifecycle.iteration}:undefined,error:c.error})),projects,recentChiefConversation:recentDm,...(dmDirectives?{chiefInstructionHistory:{...dmDirectives,items:undefined}}:{})}
