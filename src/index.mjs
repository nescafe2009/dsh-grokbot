...
export function apply(ctx, config = {}) {
    ...
    return [
      {
        ...
        output,
        async execute() {
          return JSON.stringify({ members: crewState.crew.bots.map((b) => ({ id: b.id, name: b.name, title: b.title, status: botState(b.id)?.status || 'idle' })) })
        },
      },
      {
        ...
        parameters: {
          ...
          properties: {
            ...
            persona: { type: 'string', description: 'Optional persona text' },
          },
          required: ['name', 'role'],
        },
        output,
  ...
  async function chatTurn(bot, text, { preamble = '', conversationId = null, writeDm = true, taskId = null, taskOrigin = 'continue', taskNote = '', existingRunId = null } = {}) {
    ...
    return runExclusively({ taskId, botId: bot.id, workspace: taskWs || defaultWs4Turn }, async () => {
      ...
      setTurnCtx(convKey, { taskId: taskId || null, runId: runRef?.id || null, conversationId: conversationId || null, taskWorkspace: taskWs })
