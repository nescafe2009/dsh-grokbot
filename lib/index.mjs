		...
		return [
			{
				...
				async execute(params) {
					...
					if (!c?.enabled) return JSON.stringify({ error: "Computer not configured" });
					return JSON.stringify(await sshExec(c, params.command));
				}
			},
			{
			...
			},
			{
				...
				async execute() {
					const c = await loadComputerConfig();
					if (!c?.enabled) return JSON.stringify({ error: "not configured" });
					const r = await sshExec(c, "DISPLAY=:99 import -window root /home/bot/workspace/screenshot.png 2>/dev/null && echo saved || echo no-tool");
					return JSON.stringify({
						ok: r.text.includes("saved"),
						file: "/home/bot/workspace/screenshot.png"
					});
				}
			},
			{
			...
			},
			{
				...
				async execute(params) {
					const c = await loadComputerConfig();
					if (!c?.enabled) return JSON.stringify({ error: "not configured" });
					return JSON.stringify(await sshExec(c, "cat /home/bot/workspace/" + params.path));
				}
			}
		];
	}
