...
function apply(ctx, config = {}) {
	...
	async function chiefCoordinationTurn(conversationId, retried = 0) {
		...
		try {
			...
			if (!digest) {
				releaseClaim();
				logWake({
					kind: "empty-digest-backoff",
					conversationId,
					budget: chiefWake.failAttempt(conversationId)
				});
				return;
			}
			logWake({
	...
	async function runJobBody(job, bot, convKey4Job, jobTaskIdPre) {
		{
			...
			try {
				...
				try {
					...
					timeout = setTimeout(() => session.abort.abort(/* @__PURE__ */ new Error(`job timeout after ${jobTimeoutMs}ms`)), jobTimeoutMs);
					const startedAt = Date.now();
					heartbeat = job.conversationId ? setInterval(() => {
				...
				if (runFinalStatus === "cancelled") {
					...
					await failJob(job, bot.id, "已取消（用户停止本次执行）").catch(() => void 0);
					if (job.conversationId) await appendRoomMsg(job.conversationId, {
						role: "system",
				...
				if (!reply) {
					...
					await failJob(job, bot.id, `回合报错：${outcome.error}`).catch(() => void 0);
			...
			try {
				...
				if (testEndpointsOn && method === "POST" && suffix === "/__perf/warm/open") {
					...
					const releaseLate = async (r) => {
						...
						lateReleased = true;
						try {
							r.handle.agent.cancel({ kind: "user" }, { keepInbox: true });
						} catch {}
						try {
						...
						const handleChatTurn = async () => {
							const r = await (async () => {
								...
								const result = await conversationTurn(conversation, text, {
									mentionTarget,
									taskId: bodyTaskId,
									...String(body?.evidenceMarker || "") ? { evidenceMarker: String(body.evidenceMarker) } : {}
								});
								return {
	...
	ctx.effect(() => () => {
		disposed = true;
		for (const pending of warmPending.values()) pending.aborted = true;
...
//#endregion
export { SHELL_TOOL_NAMES, activityOf, apply, src_default as default, inject, shellExecutionEvidence, summarizeTurn };
