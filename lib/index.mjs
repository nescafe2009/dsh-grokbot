...
function apply(ctx, config = {}) {
	...
	async function createBotAgent(bot, { sessionId, resume = false } = {}) {
		...
		const selection = bot.model?.provider && bot.model?.model ? bot.model : crewState.crew.defaultModel?.provider && crewState.crew.defaultModel?.model ? crewState.crew.defaultModel : null;
		const base = {
			...
			async setup(agentCtx) {
				...
				if (agentCtx.tools?.register) for (const tool of teamManagementTools(bot)) try {
