			...
			async execute(params) {
				...
				if (artifactId) {
					...
					if (task) {
						...
						if (!(conversationId ? String(artConv) === String(conversationId) : !artConv || artConv === bot.id)) return JSON.stringify({
							ok: false,
							error: `成果 ${artifactId} 来源会话不符（无归属信息或非本会话），拒绝交接`
						});
					}
	...
	function deliveryTools(bot, { conversationId }) {
		return [{
			...
			async execute(params) {
				...
				const rootReal = await realpath((activeTurnCtx.get(deliverConvKey) || null)?.taskWorkspace || botWorkspace(stateDir, bot)).catch(() => null);
				...
				if (explicitTaskId) {
					...
					turnCtx = {
						...turnCtx || {},
						taskId: chk.task.id,
						runId: chk.task.id === (turnCtx?.taskId || null) ? turnCtx?.runId ?? null : null,
						conversationId: conversationId || null,
