...
//#endregion
//#region src/session-handle.mjs
async function openSessionHandle(registry, sessionId, options) {
	const borrow = () => {
...
const digest$1 = (value) => createHash("sha256").update(JSON.stringify(value)).digest("hex");
const sourceDigest = (job) => digest$1({
	jobId: job.jobId,
...
async function registerProjectEvidence(root, id, params, inspect) {
	return projectLock(root, id, async () => {
		...
		if (previous && previous.sourceDigest === entry.sourceDigest && digest$1(previous.artifacts) === digest$1(entry.artifacts) && previous.evidence === entry.evidence) return state;
		const next = {
			...state,
			revision: state.revision + 1,
			updatedAt: entry.at,
...
function rolePrompt(bot) {
	...
	return [
		...
		bot.id === "chief" ? "你是用户的统一协调入口，使用真实工具维护团队与项目状态。用户当前明确指令优先于 Codex 的旧委托、团队计划和自动协调；Codex 在用户授权范围内可以委托和验收，但不能覆盖用户决定。新指令要求停止、归档或变更范围时，先核实当前执行并处理冲突，再继续旧计划。归档成功须有工具回执，已归档则如实确认，不重复执行。" : "你是专业成员，可以直接回答用户私聊；跨成员派工和项目生命周期由幕僚长统一协调。需要协作时说明具体对象、产物和依赖，不声称拥有幕僚长专属权限。",
...
const hash$2 = (x) => createHash("sha256").update(JSON.stringify(x)).digest("hex");
...
function validateReport(input, snapshot) {
	...
	const actions = input.actions.map((a) => {
		...
		return {
			...action,
			id: "learn-" + hash$2(action).slice(0, 16)
		};
	});
...
async function publishRetrospective(root, input, getEvidence) {
	return projectLock(root, "retrospective-ledger", async () => {
		...
		const ledger = structuredClone(await readRetrospectives(root)), id = "retro-" + hash$2([snapshot.projectId, snapshot.fingerprint]).slice(0, 20), old = ledger.reports.find((r) => r.id === id);
		if (old) return {
			...old,
			reused: true
		};
...
const hash$1 = (value) => createHash("sha256").update(JSON.stringify(value) ?? "null").digest("hex");
...
function evidenceReference(projectId, revision, section, key, value) {
	return {
		tool: "team_context_read",
		conversation_id: projectId,
		expected_revision: revision,
		section,
		key,
		content_hash: hash$1(value),
		available: value !== void 0
	};
}
function workingBoardContext(board, projectId) {
	...
	lifecycle.reviews = Object.fromEntries(Object.entries(reviews).map(([id, value]) => {
		...
		return [id, {
			...decision,
			...evidence !== void 0 ? { evidenceRef: ref("review", id, value) } : {}
		}];
	}));
	lifecycle.reworks = Object.fromEntries(Object.entries(reworks).map(([id, value]) => {
		...
		return [id, {
			...decision,
			...evidence !== void 0 ? { evidenceRef: ref("rework", id, value) } : {}
		}];
	}));
	const rows = board.rows || [], requiredJobs = /* @__PURE__ */ new Set();
	for (const row of rows.filter((r) => r.source !== "job")) for (const key of [
		"latestJobId",
		"repairJobId",
		"testJobId"
	]) if (row[key]) requiredJobs.add(row[key]);
	...
	const compactRows = selected.map((row) => {
		...
		if (checkpoint) current.checkpoint = {
			blockers: checkpoint.blockers ?? null,
			remaining: checkpoint.remaining ?? null,
			evidenceRef: ref("checkpoint", row.id, checkpoint)
		};
	...
	return {
		...
		lifecycle,
		rows: compactRows,
		executionHistory: void 0,
		contextDetail: "working",
		contextPolicyVersion: 1,
		evidenceIndex: {
			lifecycle: ref("lifecycle", "", source),
			delivery: ref("delivery", "", deliveryEvidence),
			history: ref("executionHistory", "", board.executionHistory || [])
		},
		historyOmitted: {
			taskRows: rows.length - selected.length,
			lifecycleEvents: history?.length || 0,
			iterations: iterations?.length || 0,
			note: "仅省略证据正文与已结束执行历史；原记录未改动。缺失不代表不存在，核验前按引用分页读取。"
		}
	};
}
function selectContextEvidence(board, section, key = "") {
	...
	switch (section) {
		...
		case "review": return Object.hasOwn(lifecycle.reviews || {}, key) ? lifecycle.reviews[key] : void 0;
		case "rework": return Object.hasOwn(lifecycle.reworks || {}, key) ? lifecycle.reworks[key] : void 0;
...
function contextEvidencePage(board, params) {
	...
	if (value === void 0) throw Error("CONTEXT_EVIDENCE_MISSING：证据不存在，不能推定核验通过");
	const contentHash = hash$1(value);
	if (typeof params.content_hash !== "string" || params.content_hash !== contentHash) throw Error("CONTEXT_CONTENT_CHANGED：证据已变化，重新获取引用");
	const text = JSON.stringify(value), offset = params.offset ?? 0, limit = params.limit ?? 8e3;
	if (!Number.isSafeInteger(offset) || offset < 0 || offset > text.length || !Number.isSafeInteger(limit) || limit < 1 || limit > 12e3) throw Error("上下文分页参数无效");
	...
	return {
		...
		note: "content 是原始 JSON 的分页文本；读取全部页后解析，不把部分证据当作完整结论。"
	};
}
function workingContextSize(brief) {
	const chars = JSON.stringify(brief).length;
	return {
		policyVersion: 1,
		chars,
		tokenEstimate: null,
		overTarget: chars > 32e3,
		note: chars > 32e3 ? "必要信息超过简报目标，已完整保留；不得按长度丢弃授权、范围或阻塞。" : "字符数不是 token 数；未以长度截断必要信息。"
	};
}
//#endregion
//#region src/context-maintenance.mjs
...
function contextEventIntegrity(events = []) {
	const calls = /* @__PURE__ */ new Map(), results = /* @__PURE__ */ new Map();
	...
	for (const [index, event] of events.entries()) {
		...
		if (event?.type === "tool/result") {
			...
			else if (ids.length > 1) conflictingResults.push({
				index,
				ids
			});
...
function contextDecisionManifest(brief) {
	...
	return {
		version: 2,
		hash: digest({
			projectIndex: brief?.projectIndex || [],
			projects,
			authority
		}),
...
async function compactWithDecisionGuard({ agent, compaction, signal, sourceCommandId, readBrief, events }) {
...
function userHistorySource(messages = []) {
	const source = [];
	let preceding = null;
	for (const [index, message] of messages.entries()) {
		if (message?.role === "user") {
			...
			source.push({
				id: sourceId(message, index),
				at: message.ts ?? null,
				requestId: message.requestId ?? null,
				text,
				...preceding && (text.length <= 48 || replyCue.test(text.trim())) ? { replyContext: {
					role: preceding.role,
					text: String(preceding.text || "").slice(0, 360),
					contentHash: hash(String(preceding.text || ""))
				} } : {}
			});
			preceding = null;
			continue;
...
function userDirectiveView(messages, { scope, preserveAll = false, recentLimit = 10 } = {}) {
	const source = userHistorySource(messages), seen = /* @__PURE__ */ new Map(), unique = [];
	...
	const items = unique.map((item) => {
		if (preserveAll || recentIds.has(item.id) || durableCue.test(item.text) || item.text.length <= 160 || item.replyContext) return item;
		const clipped = {
			...item,
		...
		Object.defineProperty(clipped, "fullText", {
			value: item.fullText,
			enumerable: false
		});
		return clipped;
	...
	return {
		items,
		sourceRef: {
			tool: "team_context_history_read",
			conversation_id: scope,
			message_count: source.length,
...
function compactUserDirectiveView(messages, { scope, recentLimit = 8, previewLimit = 120, maxItems = 48 } = {}) {
	const view = userDirectiveView(messages, {
		scope,
		recentLimit
	});
	...
	for (const item of [...view.items].reverse()) {
		if (selected.size >= cap) break;
	...
	const items = included.map((item) => {
		...
		return {
			...
			truncated: original.length > (recent ? 360 : previewLimit),
			occurrences: item.occurrences,
			lastAt: item.lastAt,
			...item.replyContext ? { replyContext: {
				text: item.replyContext.text.slice(0, recent ? 180 : 80),
				contentHash: item.replyContext.contentHash
			} } : {}
		};
	});
	return {
		...view,
		items,
		cataloguedMessages: items.length,
		omittedCatalogMessages: omitted.length,
		...omitted.length ? { omittedCatalog: {
			firstAt: omitted[0].at,
			lastAt: omitted.at(-1).at,
			contentHash: hash(omitted.map((item) => ({
				id: item.id,
				at: item.at,
				contentHash: hash(String(item.fullText ?? item.text ?? ""))
			})))
		} } : {},
		truncatedMessages: items.filter((item) => item.truncated).length,
		note: "跨作用域只注入有总条目上限的来源目录；精确原文通过 sourceRef 或当前问题的确定性检索恢复。目录记录不得被解释为当前项目的新授权。"
	};
}
function contextHistoryPage(messages, params) {
	...
	const text = JSON.stringify(source), offset = params.offset ?? 0, limit = params.limit ?? 8e3;
	if (!Number.isSafeInteger(offset) || offset < 0 || offset > text.length || !Number.isSafeInteger(limit) || limit < 1 || limit > 12e3) throw Error("上下文分页参数无效");
	const end = Math.min(text.length, offset + limit);
	return {
		contentHash,
		...
		complete: end === text.length,
		nextOffset: end < text.length ? end : null,
		note: "content 是用户消息来源记录的原始 JSON 分页；完整读取后再解析。"
	};
}
const retrievalStop = /* @__PURE__ */ new Set([
	"上下",
	"上下文",
	"历史",
	"之前",
...
function retrievalTerms(text) {
	...
	for (const run of normalized.match(/[\u3400-\u9fff]{3,}/g) || []) for (let size = 2; size <= 4; size += 1) for (let i = 0; i + size <= run.length; i += 1) {
		const term = run.slice(i, i + size);
		if (!retrievalStop.has(term)) terms.add(term);
	}
	return terms;
}
...
function relevantUserHistory(messages, query, { excludeRequestId = null, limit = 6 } = {}) {
	...
	for (const record of userHistorySource(messages)) {
		...
		const textTerms = retrievalTerms(record.text), contextTerms = retrievalTerms(record.replyContext?.text || ""), terms = /* @__PURE__ */ new Set([...textTerms, ...contextTerms]);
		let score = 0, common = 0, textCommon = 0;
		for (const term of wanted) if (textTerms.has(term)) {
			common += 1;
			textCommon += 1;
			score += (term.length >= 4 ? 4 : term.length === 3 ? 2 : 1) * 3;
	...
	return scored.sort((a, b) => b.score - a.score || (b.record.at || 0) - (a.record.at || 0)).slice(0, Math.max(1, Math.min(12, limit))).sort((a, b) => (a.record.at || 0) - (b.record.at || 0)).map(({ record, score }) => ({
		...record,
		score,
		text: record.text.slice(0, 1600),
		contentLength: record.text.length,
		contentHash: hash(record.text),
		truncated: record.text.length > 1600
	}));
}
//#endregion
//#region src/session-scope.mjs
function parseSessionKey(value) {
	...
	return {
		sessionKey: key,
		botId,
		conversationId: conversation === botId ? null : conversation,
		workspace: marker >= 0 ? key.slice(marker + 4) : null
	};
}
function resolveSessionOwner(entries, sessionId) {
	const found = [...entries].find(([, sid]) => String(sid) === String(sessionId));
	return found ? parseSessionKey(found[0]) : null;
}
//#endregion
//#region src/context-metrics.mjs
...
function contextMetrics(events, now = Date.now()) {
	const rows = /* @__PURE__ */ new Map(), tools = /* @__PURE__ */ new Map();
	let model = null, provider = null;
	const get = (d) => rows.get(`${d.turn}:${d.step}`);
	for (const e of events) {
		const d = e.data || {}, at = num(e.time);
		if (e.type === "request/header") {
			model = safe(d.header?.config?.model);
			provider = safe(d.header?.config?.provider);
		}
		if (e.type === "step/start" && !get(d) && Number.isInteger(d.turn) && Number.isInteger(d.step)) rows.set(`${d.turn}:${d.step}`, {
			turn: d.turn,
			step: d.step,
			startedAt: at,
			endedAt: null,
		...
		if (r) {
			...
			if (e.type === "assistant/chunk" && [
				...
				"reasoning-delta",
				"tool-call-delta"
			].includes(d.chunk?.type) && a.firstAt === null) a.firstAt = at;
			if (e.type === "llm/retry") {
				a.endedAt = at;
				a.outcome = [
					"TIMEOUT",
					...
					"RATE_LIMIT",
					"EMPTY_RESPONSE"
				].includes(d.failure?.code) ? d.failure.code : "UNKNOWN_ERROR";
			}
			if (e.type === "llm/retry-started") r.attempts.push({
				startedAt: at,
				firstAt: null,
				endedAt: null,
				outcome: "pending"
			...
			if (e.type === "step/end") {
				r.endedAt = at;
				if (a.endedAt === null) a.endedAt = at;
			}
		}
		if (e.type === "tool/call" && r) tools.set(d.callId, {
			at,
			row: r
		});
		if (e.type === "tool/result") {
	...
	return [...rows.values()].map((r) => {
		...
		return {
			...r,
			inputReported: known.length ? known.reduce((a, b) => a + b, 0) : null,
			inputFieldsReported: r.usage ? [
				"inputTokens",
				"cacheReadTokens",
				"cacheWriteTokens"
			].filter((k) => r.usage[k] !== null) : [],
			firstResponseMs: duration(r.startedAt, first),
			elapsedMs: duration(r.startedAt, r.endedAt ?? now),
			retryCount: Math.max(0, r.attempts.length - 1),
			attempts: r.attempts.map((a) => ({
				...a,
				firstResponseMs: duration(a.startedAt, a.firstAt),
				elapsedMs: duration(a.startedAt, a.endedAt ?? now)
			}))
		};
	});
}
async function saveContextMetrics(stateDir, botId, sessionId, events) {
	if (!/^[\w-]+$/.test(botId) || !/^[a-f0-9-]{36}$/i.test(sessionId)) throw Error("指标归属无效");
	const dir = join(stateDir, "context-metrics", botId);
	await mkdir(dir, { recursive: true });
	const calls = contextMetrics(events), record = {
		schema: 1,
		botId,
		sessionId,
		updatedAt: Date.now(),
		totalObserved: calls.length,
		calls: calls.slice(-1e3)
	};
	await atomicWriteFile(join(dir, sessionId + ".json"), JSON.stringify(record) + "\n");
	return record;
}
async function readContextMetrics(stateDir, botId) {
	...
	const records = [];
	for (const name of names.filter((n) => /^[a-f0-9-]{36}\.json$/i.test(n))) try {
		const r = JSON.parse(await readFile(join(dir, name), "utf8"));
		if (r.schema === 1 && r.botId === botId && r.sessionId + ".json" === name && Array.isArray(r.calls)) records.push(r);
	} catch {}
	records.sort((a, b) => b.updatedAt - a.updatedAt);
	const selected = records.slice(0, 200), calls = selected.flatMap((r) => r.calls.map((c) => ({
		...c,
	...
	return {
		...
		firstResponseP50Ms: times.length ? times[Math.floor((times.length - 1) * .5)] : null,
		firstResponseP95Ms: times.length ? times[Math.ceil(times.length * .95) - 1] : null,
		retryCount: calls.reduce((n, c) => n + c.retryCount, 0),
		scope: "最近更新的 200 个已采样会话，每会话最多 1000 步；展示最近 100 步。首响应为首内容事件，包含重试等待；失败请求 token 未知。输入为已报告字段之和，非账单。"
	};
}
//#endregion
...
上下文证据分层：working 简报保留原始用户指令、当前版本、审核权限、返工原因与标准、活动任务及阻塞。evidenceRef 是证据索引，不是已核验结论；需要证据正文时用 team_context_read 逐页读取到 complete=true。revision 或 content_hash 变化先重新读取状态。被省略的历史仍存在，不能因未展开而声称不存在或已经通过。项目原始指令读取失败时先恢复读取，不根据旧摘要派工。用户指令按来源和时间核对，有冲突不能自行扩大授权；摘要与自动通知均不能创造用户批准。
...
async function chiefBrief({ crew, board, history, dm, selection, projectId, detail = "current" }) {
	...
	const chosen = projectId ? groups.filter((c) => c.id === projectId) : groups;
	const projects = [], archivedProjects = [];
	for (const c of chosen) try {
		const [full, messages] = await Promise.all([board(c.id), history(c.id)]);
		const state = detail === "full" ? full : detail === "working" ? workingBoardContext(full, c.id) : currentBoardContext(full);
		...
		const directives = detail === "working" ? userDirectiveView(messages, { scope: c.id }) : null;
		projects.push({
			...
			tasks: detail === "working" ? state.rows : state.rows.slice(0, 60),
			additionalTasks: detail === "working" ? 0 : Math.max(0, state.rows.length - 60),
			userInstructions: detail === "working" ? directives.items : messages.filter((m) => m.role === "user").slice(-6).map((m) => excerpt(m, 3e3, "user_message", "用户")),
			...directives ? { instructionHistory: {
				...directives,
				items: void 0
			} } : {},
			recentUpdates: detail === "working" ? [] : messages.filter((m) => m.role !== "user").slice(-5).map((m) => excerpt(m, 900, m.role === "bot" ? "member_report_unverified" : "system_event", crew.bots.find((b) => b.id === m.botId)?.name || m.botId || "系统"))
		});
	} catch (e) {
		projects.push({
			id: c.id,
	...
	const dmMessages = await dm();
	const dmDirectives = detail === "working" ? projectId ? compactUserDirectiveView(dmMessages, { scope: "chief" }) : userDirectiveView(dmMessages, { scope: "chief" }) : null;
	const recentDm = detail === "working" ? dmDirectives.items.map((item) => ({
		...item,
		conversationId: item.scope,
		scope: "幕僚长私聊；逐条核对适用项目，不是当前项目的新授权"
	})) : dmMessages.slice(-8).map((m) => excerpt(m, 1800, m.role === "user" ? "user_message" : m.role === "bot" ? "chief_previous_reply_unverified" : "system_event", m.role === "user" ? "用户" : m.role === "bot" ? "幕僚长" : "系统"));
	const result = {
		archivedProjects: projectId ? archivedProjects : archivedProjects.slice(-12),
		...
		...dmDirectives ? { chiefInstructionHistory: {
			...dmDirectives,
			items: void 0
		} } : {}
	};
	if (detail === "working") result.contextBudget = workingContextSize(result);
	return result;
}
...
function chain(map, key, fn, priority = 0) {
	...
	return new Promise((resolve, reject) => {
		...
		const pump = () => {
			...
			if (!item) {
				if (map.get(key) === queue) map.delete(key);
				return;
			}
			queue.running = true;
...
function withTaskLock(taskId, fn, priority = 0) {
	if (!taskId) return fn();
...
*/
function runExclusively({ taskId = null, botId, workspace = null, priority = 0 }, fn) {
...
function apply(ctx, config = {}) {
	...
	const contextMaintenanceRuns = /* @__PURE__ */ new Map();
	const authorityAssemblyStats = /* @__PURE__ */ new Map();
	const pendingJobs = [];
	...
	const contextMaintenancePath = join(stateDir, "context-maintenance.jsonl");
	...
	function ownerOfSession(sessionId) {
		return resolveSessionOwner(chatSessionIds.entries(), sessionId);
	}
	async function recordContextMaintenance(entry) {
		const messageId = entry.messageId || (entry.kind === "request" && entry.requestId ? `context-request-${entry.requestId}-${entry.status}` : entry.compactionId ? `context-event-${entry.compactionId}-${entry.type}` : void 0);
		await appendTranscript(contextMaintenancePath, {
			...entry,
			at: entry.at || Date.now(),
			...messageId ? { messageId } : {}
		});
	}
	async function readContextMaintenance(botId, limit = 40) {
		try {
			return (await readFile(contextMaintenancePath, "utf8")).split("\n").filter(Boolean).map((line) => JSON.parse(line)).filter((entry) => entry.botId === botId).slice(-limit);
		} catch {
			return [];
		}
	}
	async function contextMaintenanceRequest(requestId) {
		try {
			return (await readFile(contextMaintenancePath, "utf8")).split("\n").filter(Boolean).map((line) => JSON.parse(line)).filter((entry) => entry.requestId === requestId && entry.kind === "request").at(-1) || null;
		} catch (error) {
			if (error?.code === "ENOENT") return null;
			throw Error(`CONTEXT_MAINTENANCE_LEDGER_UNAVAILABLE：${safeError(error)}`);
		}
	}
	async function readRoomMsgs(roomId, limit = 200, strict = false) {
		try {
			...
			if (strict && error?.code !== "ENOENT") throw Error("项目原始指令读取失败；不能推定没有约束");
			return [];
		}
	}
	...
	async function contextBoard(id) {
		return projectBoard({
			stateDir,
			inboxRoot,
			conversationId: id,
	...
	async function chiefProjectContext(projectId = null, detail = "working") {
		const brief = await chiefBrief({
			...
			board: contextBoard,
			history: (id) => readRoomMsgs(id, detail === "working" ? Number.MAX_SAFE_INTEGER : 80, detail === "working"),
			dm: () => readDm("chief", detail === "working" ? Number.MAX_SAFE_INTEGER : 8, detail === "working")
		});
	...
	async function sessionAuthorityBrief(bot, conversationId = null) {
		if (bot.id === "chief") {
			const brief = await chiefProjectContext(conversationId || null);
			brief.sessionScope = {
				botId: bot.id,
				conversationId: conversationId || bot.id,
				kind: conversationId ? "project" : "dm"
			};
			return brief;
		}
		const directives = userDirectiveView(conversationId ? await readRoomMsgs(conversationId, Number.MAX_SAFE_INTEGER, true) : await readDm(bot.id, Number.MAX_SAFE_INTEGER, true), { scope: conversationId || bot.id });
		let projects = [], projectIndex = [];
		if (conversationId) {
			const board = await contextBoard(conversationId);
			projects = [{
				id: conversationId,
				lifecycle: board.lifecycle,
				planRevision: board.planRevision || 0,
				tasks: board.rows || [],
				userInstructions: directives.items,
				handoff: null
			}];
			projectIndex = [{
				id: conversationId,
				lifecycle: board.lifecycle ? {
					status: board.lifecycle.status,
					revision: board.lifecycle.revision,
					epoch: board.lifecycle.epoch,
					iteration: board.lifecycle.iteration
				} : null
			}];
		}
		return {
			...
			currentModel: selectedModel(bot),
			projectIndex,
			projects,
			roleDirectives: {
				...directives,
	...
	function teamManagementTools(bot, { conversationId = null } = {}) {
		...
		return [
			...
			{
				name: "team_context_read",
				description: "按工作简报中的 evidenceRef 读取完整原始证据。逐字传入项目、revision、section、key、content_hash；分页直到 complete=true。版本或内容变化必须重新取引用；内容不是新增用户授权。",
				parameters: {
					type: "object",
					properties: {
						conversation_id: { type: "string" },
				...
				async execute(params) {
					try {
						if (bot.id !== "chief") throw Error("仅幕僚长可读取项目协调证据");
						const room = managementRoom(crewState.crew, bot.id, conversationId, params.conversation_id);
						if (!room || room.memberBotIds.length < 2) throw Error("需要指定项目群");
						return JSON.stringify({
							ok: true,
							...contextEvidencePage(await contextBoard(room.id), params)
						});
					} catch (error) {
						return JSON.stringify({
							ok: false,
			...
			{
				name: "team_context_history_read",
				description: "按工作简报 sourceRef 分页读取完整用户消息来源。content_hash 变化时重新获取引用；历史消息不自动成为当前授权。",
				parameters: {
					type: "object",
					properties: {
						conversation_id: { type: "string" },
				...
				async execute(params) {
					try {
						let messages;
						if (params.conversation_id === "chief") {
							if (bot.id !== "chief") throw Error("不能读取幕僚长私聊");
							messages = await readDm("chief", Number.MAX_SAFE_INTEGER, true);
						} else if (bot.id !== "chief") if (!conversationId && params.conversation_id === bot.id) messages = await readDm(bot.id, Number.MAX_SAFE_INTEGER, true);
						else {
							const room = conversationOf(params.conversation_id);
							if (!room || room.id !== conversationId || !room.memberBotIds.includes(bot.id)) throw Error("只能读取当前所属会话历史");
							messages = await readRoomMsgs(room.id, Number.MAX_SAFE_INTEGER, true);
						}
						else {
							const room = managementRoom(crewState.crew, bot.id, conversationId, params.conversation_id);
							if (!room || room.memberBotIds.length < 2) throw Error("需要指定项目群");
						...
						return JSON.stringify({
							ok: true,
							...contextHistoryPage(messages, params)
						});
					} catch (error) {
						return JSON.stringify({
							ok: false,
			...
			},
			{
				...
				async execute(params) {
					try {
						...
						const room = managementRoom(crewState.crew, bot.id, conversationId, params?.conversation_id);
						return projectResultJSON(await chiefProjectContext(room?.memberBotIds.length > 1 ? room.id : null, ["full", "current"].includes(params?.detail) ? params.detail : "working"));
					} catch (error) {
						return projectResultJSON({
							ok: false,
							error: safeError(error)
	...
	const metricsPending = /* @__PURE__ */ new Map();
	function persistMetrics(session) {
		const pending = (metricsPending.get(session) || Promise.resolve()).then(() => saveContextMetrics(stateDir, session.botId, session.nativeSessionId, sessionEvents(session.handle.agent.session))).catch((error) => ctx.logger?.warn?.(`grokbot context metrics: ${safeError(error)}`));
		metricsPending.set(session, pending);
		pending.finally(() => {
			if (metricsPending.get(session) === pending) metricsPending.delete(session);
		});
		return pending;
	}
	const metricsTimer = setInterval(() => {
		for (const session of activeSessions) if (!metricsPending.has(session)) persistMetrics(session);
	}, 15e3);
	...
	async function createBotAgent(bot, { sessionId, resume = false, conversationId = null, cwd = null } = {}) {
		...
		let handle;
		try {
			if (resume && sessionId) handle = await openSessionHandle(ctx.agents, sessionId, {
	...
	ctx.effect(() => ctx.on("system-prompt/assemble", async (assembly, context, next) => {
		...
		const sessionId = context?.sessionId || context?.session?.id || context?.agent?.session?.id || "";
		if (!sessionId) return resolved;
		let botId = null;
		let conversationId = null;
		const mapped = ownerOfSession(sessionId);
		if (mapped) {
			botId = mapped.botId;
		...
		const authorityIndex = sections.findIndex((s) => s.name === "grokbot:authority-context");
		if (authorityIndex >= 0) sections.splice(authorityIndex, 1);
		try {
			if (bot.id === "chief") {
				const authority = await sessionAuthorityBrief(bot, conversationId);
				const authorityText = CHIEF_CONTEXT_RULES + "\n" + JSON.stringify(authority);
				sections.push({
					name: "grokbot:authority-context",
					order: -15,
					text: authorityText
				});
				authorityAssemblyStats.set(String(sessionId), {
					at: Date.now(),
					botId: bot.id,
					conversationId: conversationId || null,
					chars: authorityText.length,
					manifest: contextDecisionManifest(authority).hash
				});
			} else {
				const authority = await sessionAuthorityBrief(bot, conversationId);
				const authorityText = "【用户指令来源索引】以下记录按原始时序生成；status=recorded 不代表仍然生效。遇到冲突、撤销或范围变化时按最新明确用户消息处理；截断项必须用 sourceRef 读取全文。\n" + JSON.stringify(authority);
				sections.push({
					name: "grokbot:authority-context",
					order: -15,
					text: authorityText
			...
			ctx.logger?.error?.(`grokbot authority context unavailable for ${bot.id}: ${safeError(error)}`);
			throw Error(`CONTEXT_AUTHORITY_UNAVAILABLE：${safeError(error)}`);
		}
		if (!hadIdentity) try {
	...
	ctx.effect(() => ctx.on("session/event", (session, event) => {
		...
		const owner = ownerOfSession(session?.id);
		if (!owner) return;
		...
		recordContextMaintenance({
			...
			trigger: data.sourceCommandId || data.turn === null ? "manual" : "automatic",
			turn: data.turn ?? null,
			...event.type === "compaction/summary" ? {
				summarySeq: event.seq ?? null,
				shadowedItems: Array.isArray(data.shadowedSeqs) ? data.shadowedSeqs.length : null,
				shadowedTokens: data.shadowedTokenCount ?? null,
				shadowedRange: data.shadowedRange || null,
				provider: data.provider || null,
				model: data.model || null
			} : {},
			...event.type === "compaction/end" ? { error: data.error ? safeError(data.error) : null } : {},
			replaceGeneration: session.surface?.replaceGeneration ?? null
		}).catch((error) => ctx.logger?.warn?.(`上下文维护记录失败：${safeError(error)}`));
	}), "grokbot: context maintenance observability");
	ctx.effect(() => ctx.on("agent/created", (ev) => {
		const agent = ev?.agent ?? ev;
		if (!agent?.ctx?.tools?.register || !agent?.session?.id) return;
		const owner = ownerOfSession(agent.session.id);
		const convId = owner?.conversationId ?? null, botId = owner?.botId ?? null;
		if (!botId) return;
		const bot = crewState.crew.bots.find((b) => b.id === botId);
		if (!bot) return;
		for (const tool of [
	...
	ctx.effect(() => ctx.on("agent/pre-step", async (request, next) => {
		...
		const botId = ownerOfSession(id)?.botId ?? baseline?.botId;
		try {
	...
	async function readDm(botId, limit = 200, strict = false) {
		try {
			...
			if (strict && error?.code !== "ENOENT") throw Error("私聊原始指令读取失败；不能推定没有约束");
			return [];
		}
	}
	async function chatTurn(bot, text, { commandActor = "user", preamble = "", conversationId = null, writeDm = true, userMessageWritten = false, requestId = null, taskId = null, taskOrigin = "continue", taskNote = "", existingRunId = null, evidenceMarker = "" } = {}) {
		...
		return runExclusively({
			...
			priority: typeof text === "string" ? commandActor === "codex" ? 50 : 100 : 0
		}, async () => {
			...
			try {
				...
				if (!(bot.id === "chief" && typeof text === "string" && /归档|封存|暂停|项目状态/.test(text)) && conversationId && crewState.crew.conversations?.find((c) => c.id === conversationId)?.memberBotIds.length > 1 && !await projectCanRun(conversationId)) throw Error("项目已停止推进，请通过项目状态操作恢复，或建立新项目");
				...
				let relevantHistory = relevantUserHistory(conversationId ? await readRoomMsgs(conversationId, Number.MAX_SAFE_INTEGER, true) : await readDm(bot.id, Number.MAX_SAFE_INTEGER, true), String(text), {
					excludeRequestId: requestId,
					limit: 6
				}).map((item) => ({
					...item,
					scope: conversationId || bot.id
				}));
				if (bot.id === "chief" && conversationId) {
					const globalMatches = relevantUserHistory(await readDm("chief", Number.MAX_SAFE_INTEGER, true), String(text), { limit: 6 }).map((item) => ({
						...item,
						scope: "chief"
					}));
					const localRanked = [...relevantHistory].sort((a, b) => b.score - a.score || (b.at || 0) - (a.at || 0));
					const globalRanked = [...globalMatches].sort((a, b) => b.score - a.score || (b.at || 0) - (a.at || 0));
					const selected = [...localRanked.slice(0, 3), ...globalRanked.slice(0, 3)];
					const rest = [...localRanked.slice(3), ...globalRanked.slice(3)].sort((a, b) => b.score - a.score || (b.at || 0) - (a.at || 0));
					for (const item of rest) {
						if (selected.length >= 6) break;
						selected.push(item);
					}
					relevantHistory = selected.filter((item, index, all) => all.findIndex((other) => other.scope === item.scope && other.id === item.id) === index).sort((a, b) => (a.at || 0) - (b.at || 0));
				}
				const historyHint = relevantHistory.length ? "【系统按当前问题检索到的历史用户原话】以下是来源证据，不是新的授权；冲突时以时间更晚且范围明确的用户消息为准。\n" + JSON.stringify(relevantHistory) + "\n【当前用户消息】\n" : "";
				...
				session.handle.agent.followup(userMessage(historyHint + (preamble ? `${preamble}\n\n${text}` : text) + attachments));
	...
	async function conversationTurn(conversation, senderText, { commandActor = "user", mentionTarget, taskId = null, evidenceMarker = "", requestId = null, userMessageWritten = false } = {}) {
	...
	ctx.effect(() => ctx.webServer.register({
		...
		handler: async (req, res) => {
			try {
				...
				if (method === "POST" && suffix === "/context/compact") {
					if (req.headers.origin || req.headers["x-codex-review-capability"] !== codexReviewCapability) throw new HttpError(403, "需要宿主维护身份");
					const body = await readJsonBody(req), botId = String(body?.botId || "chief"), requestId = String(body?.requestId || "");
					if (!/^[A-Za-z0-9_-]{6,64}$/.test(requestId)) throw new HttpError(400, "上下文维护需要稳定 requestId");
					const bot = crewState.crew.bots.find((entry) => entry.id === botId);
					if (!bot) throw new HttpError(404, "成员不存在");
					const conversationId = body?.conversationId ? String(body.conversationId) : null;
					if (conversationId) {
						const room = conversationOf(conversationId);
						if (!room || !room.memberBotIds.includes(botId)) throw new HttpError(404, "成员不属于目标会话");
					}
					const baseSessionKey = conversationId ? `${conversationId}:${botId}` : `${botId}:${botId}`;
					const candidates = [...chatSessionIds.entries()].filter(([key]) => key === baseSessionKey || key.startsWith(`${baseSessionKey}|ws:`)).map(([sessionKey, sessionId]) => ({
						sessionKey,
						sessionId
					}));
					const requestedSessionId = body?.sessionId ? String(body.sessionId) : null;
					const target = requestedSessionId ? candidates.find((entry) => entry.sessionId === requestedSessionId) : candidates.length === 1 ? candidates[0] : null;
					if (requestedSessionId && !target) throw new HttpError(409, "指定 sessionId 不属于该成员与会话");
					if (!requestedSessionId && candidates.length > 1) throw new HttpError(409, "该成员与会话存在多个 workspace session；请指定 sessionId");
					if (!target) throw new HttpError(409, "成员尚无持久会话");
					const { sessionKey, sessionId } = target;
					const payloadHash = createHash("sha256").update(JSON.stringify({
						botId,
						conversationId: conversationId || null,
						sessionId
					})).digest("hex");
					const existing = contextMaintenanceRuns.get(requestId);
					if (existing) {
						if (existing.payloadHash !== payloadHash) throw new HttpError(409, "CONTEXT_MAINTENANCE_REQUEST_COLLISION");
						respond(res, 200, {
					...
					const run = (async () => {
						const prior = await contextMaintenanceRequest(requestId);
						if (prior) {
							if (prior.payloadHash !== payloadHash) throw new HttpError(409, "CONTEXT_MAINTENANCE_REQUEST_COLLISION");
							if (prior.status === "completed") return {
								...prior.result,
								deduped: true
							};
							throw new HttpError(409, `上下文维护原请求状态为 ${prior.status}；禁止盲目重试，请查询维护记录`);
						}
						if (botState(botId).status !== "idle" || [...activeTurnCtx.values()].some((turn) => String(turn.sessionKey || "") === sessionKey || String(turn.sessionKey || "").startsWith(`${sessionKey}|`))) throw new HttpError(409, "成员仍在执行，不能压缩上下文");
						const sourceCommandId = `grokbot-${requestId}`;
						await recordContextMaintenance({
							kind: "request",
							requestId,
							payloadHash,
							status: "started",
						...
						let session = chatHandles.get(sessionKey), owned = false;
						const abort = new AbortController(), timer = setTimeout(() => abort.abort(/* @__PURE__ */ new Error("上下文维护超时")), 18e4);
						try {
							if (!session) {
								const marker = sessionKey.indexOf("|ws:"), cwd = marker >= 0 ? sessionKey.slice(marker + 4) : void 0;
								session = await createBotAgent(bot, {
									sessionId,
									resume: true,
									conversationId,
							...
							const compacted = await compactWithDecisionGuard({
								agent: session.handle.agent,
								compaction,
								signal: abort.signal,
								sourceCommandId,
								readBrief: () => sessionAuthorityBrief(bot, conversationId),
								events: () => sessionEvents(session.handle.agent.session)
							});
							await persistMetrics(session);
							const result = {
								ok: true,
							...
							const status = error?.committed ? "committed_validation_failed" : "failed";
							await recordContextMaintenance({
								kind: "request",
								requestId,
								payloadHash,
								...
								summarySeq: error?.compactionResult?.summarySeq ?? null
							});
							throw error;
						} finally {
							clearTimeout(timer);
							if (owned) await session.dispose();
						}
					})();
					const slot = {
						payloadHash,
					...
					contextMaintenanceRuns.set(requestId, slot);
					try {
						respond(res, 200, await run);
						return;
					} finally {
						if (contextMaintenanceRuns.get(requestId) === slot) contextMaintenanceRuns.delete(requestId);
					}
				}
				...
				if (method === "GET" && workMatch) {
					...
					if (detail) {
						for (const session of activeSessions) if (session.botId === bot.id && !metricsPending.has(session)) persistMetrics(session);
						snapshot.efficiency = await readContextMetrics(stateDir, bot.id);
						snapshot.contextMaintenance = await readContextMaintenance(bot.id);
						snapshot.authorityContext = [...authorityAssemblyStats.entries()].filter(([, value]) => value.botId === bot.id).map(([sessionId, value]) => ({
							sessionId,
							...value
						})).slice(-20);
						snapshot.conversations = [...chatHandles.entries()].filter(([, session]) => ownerOfSession(session.handle.agent.session.id)?.botId === bot.id).map(([key, session]) => ({
							id: key,
				...
				if (method === "POST" && suffix === "/bots") {
					...
					try {
						...
						await persistChatSessions();
						await (await createBotAgent(bot, { sessionId })).dispose();
					} catch (error) {
						...
						const handleChatTurn = async () => {
							...
							const r = await (async () => {
								...
								const result = await conversationTurn(conversation, text, {
									commandActor: !req.headers.origin && req.headers["x-codex-review-capability"] === codexReviewCapability ? "codex" : "user",
									mentionTarget,
	...
	ctx.effect(() => async () => {
		...
		await Promise.allSettled([...activeSessions].map((session) => session.dispose()));
	}, "grokbot: shutdown");
}
var src_default = {
	name: "grokbot",
