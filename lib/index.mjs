		...
		return [
			...
			},
			{
				...
				async execute(params) {
					...
					const r = await sshExec(c, `if ! pgrep -u "$USER" chromium >/dev/null 2>&1 || ! pgrep -f "chromium.*${p.replace(/[^a-z0-9/._-]/gi, "")}" >/dev/null 2>&1; then DISPLAY=:99 sh -c '(chromium-browser || chromium) --no-sandbox --start-maximized "${url}" >/dev/null 2>&1 &' ; fi`, 2e4);
					return JSON.stringify({
						ok: r.ok,
						url,
