...
function apply(ctx, config = {}) {
	...
	async function conversationTurn(conversation, senderText, { mentionTarget } = {}) {
		...
		const outcome = await chatTurn(responder, senderText, { preamble: [`【群聊 ${conversation.name}】成员：${members.map((bot) => `${bot.avatar}${bot.name}`).join("、")}。`, "你现在在群聊中应答用户消息。若你认为某条工作应由其他成员处理，在回复的最后一行单独写「@成员名 交代内容」，系统会异步转交；不要除此行外提交接。"].join("\n") });
					...
					for (const bot of crewState.crew.bots) {
						...
						if (!roleTemplate) {
							for (const [prefix, key] of [
								...
								["秘书", "secretary"],
								["审核官", "reviewer"]
							]) if (bot.title && bot.title.startsWith(prefix)) {
								roleTemplate = key;
								break;
							}
						}
