...
function apply(ctx, config = {}) {
	...
	const chatRequestCache = /* @__PURE__ */ new Map();
	const pendingJobs = [];
	...
	async function runInboxJob(job) {
		...
		const execWs = (jobTaskIdPre ? (await getTask(stateDir, jobTaskIdPre).catch(() => null))?.workspace || null : null) || botWorkspace(stateDir, bot);
	...
	function pump() {
		...
		while (runningJobs.size + waitingJobs.size < maxConcurrentJobs && pendingJobs.length > 0) {
			const busy = /* @__PURE__ */ new Set([...[...runningJobs.values()].map((entry) => entry.botId), ...[...waitingJobs.values()].map((w) => w.job.toBot)]);
				...
				if (convMatch) {
					...
					if (method === "POST" && convMatch[2] === "chat") {
						...
						if (requestId) {
							...
							if (cached && Date.now() - cached.at < 5 * 6e4) {
								respond(res, 200, {
									...cached.result,
									deduped: true
								});
						...
						if (requestId) chatRequestCache.set(requestId, {
							at: Date.now(),
							result: {
