...
function apply(ctx, config = {}) {
	...
	async function createBotAgent(bot, { sessionId, resume = false, conversationId = null } = {}) {
		...
		const base = {
			...
			async setup(agentCtx) {
				...
				if (agentCtx.tools?.register) for (const tool of [...teamManagementTools(bot, { conversationId }), ...computerTools(bot)]) try {
	...
	ctx.effect(() => ctx.on("agent/created", (ev) => {
		...
		const bot = crewState.crew.bots.find((b) => b.id === botId);
		if (!bot) return;
		for (const tool of [...teamManagementTools(bot, { conversationId: convId }), ...computerTools(bot)]) try {
