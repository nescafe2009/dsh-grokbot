...
const taskQueues = /* @__PURE__ */ new Map();
/**
...
function apply(ctx, config = {}) {
		...
		return [{
			...
			async execute(params) {
				...
				return JSON.stringify({
					ok: true,
					taskId: task.id,
					note: "本回合及后续 deliver_file 会自动关联此任务；续改请在完成后告知用户可在卡片上「继续修改」"
				});
			}
		}, {
			name: "handoff",
			...
			async execute(params) {
				...
				if (taskId && !task) return JSON.stringify({
					ok: false,
					error: `任务不存在：${taskId}`
				});
				if (task) {
					const taskConv = task.conversationId;
					if (!(!conversationId ? taskConv === null || task.ownerBotId === bot.id : taskConv === conversationId)) return JSON.stringify({
						ok: false,
						error: `任务 ${taskId} 不属于当前会话，拒绝跨会话交接`
					});
				}
				...
				if (artifactId) {
					...
					if (task && !task.artifacts.includes(artifactId)) return JSON.stringify({
						ok: false,
		...
		return [{
			...
			async execute(params) {
				...
				if (classifyDeliveryTarget({
					...
					botId: bot.id,
					conversations: crewState.crew.conversations
				}) === "rejected") return `ERROR: 目标会话 ${conversationId} 已不存在（可能已被删除），已取消交付：${basename(real)}。请告知用户重新建会话后重新交付。`;
				...
				if (explicitTaskId) {
					...
					const taskConv = task.conversationId;
					if (!(!conversationId ? true : taskConv === conversationId)) return `ERROR: 任务 ${explicitTaskId} 不属于当前会话，拒绝关联`;
					turnCtx = {
						...turnCtx || {},
						taskId: task.id,
						conversationId: conversationId || null
	...
	const botTurnQueues = /* @__PURE__ */ new Map();
	function serializeBotTurn(botId, run) {
		const next = (botTurnQueues.get(botId) ?? Promise.resolve()).then(run, run);
		botTurnQueues.set(botId, next.catch(() => void 0));
		return next;
	}
	...
	async function conversationTurn(conversation, senderText, { mentionTarget, taskId = null } = {}) {
		...
		if (handoff) {
			...
			if (!target) target = eligibleBots(conversation).find((b) => b.id === handoff[1] || b.name === handoff[1]) ?? null;
			if (!target) appendRoomMsg(conversation.id, {
				role: "system",
				text: `「@${handoff[1]}」交接未执行：${resolvedHandoff.error || "目标不在本群成员内"}。请使用 handoff 工具（精确 member_id）。`
			}).catch(() => void 0);
			if (target && target.id !== responder.id) {
				...
				(async () => {
					try {
						...
						const relay = await chatTurn(target, handoff[2], {
