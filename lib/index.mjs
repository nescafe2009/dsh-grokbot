	...
	function computerTools(bot) {
		...
		return [
			...
			},
			{
				...
				description: "Get a preview URL for a deliverable on the team computer (HTML game/page etc.) and open it in the user's browser. path is relative to the shared workspace. Always prefer this over local `open` commands — the shared computer files are not on this Mac.",
				parameters: {
					type: "object",
					properties: { path: {
						type: "string",
				...
				output,
				async execute(params) {
					if (!(await loadComputerConfig())?.enabled) return "Computer not configured";
