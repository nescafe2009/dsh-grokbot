...
function apply(ctx, config = {}) {
	...
	function localExec(command, timeoutMs) {
		return new Promise((resolve) => {
			...
			const child = spawn("/bin/bash", ["-c", command], { stdio: [
				"ignore",
				"pipe",
				"pipe"
			] });
			...
			},
			{
				...
				async execute(params) {
					try {
						...
						const conversationId = turnContexts.get(bot.id)?.conversationId || void 0;
	...
	const turnContexts = /* @__PURE__ */ new Map();
	...
	const chiefWakeLast = /* @__PURE__ */ new Map();
	function wakeChiefForGroup(conversationId, fromBotId = null) {
		try {
			...
			if (Date.now() - (chiefWakeLast.get(conversationId) || 0) < 6e4) return;
	...
	async function runInboxJob(job) {
		...
		try {
			...
			if (job.conversationId) turnContexts.set(bot.id, {
				conversationId: job.conversationId,
				convKey: `${job.conversationId}:${bot.id}`,
				taskId: null,
				botId: bot.id
			});
