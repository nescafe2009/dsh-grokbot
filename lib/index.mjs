	...
	function deliveryTools(bot, { conversationId }) {
		return [{
			...
			async execute(params) {
				...
				if (conversationId && conversationId !== bot.id && !conv) return `ERROR: 目标会话 ${conversationId} 已不存在（可能已被删除），已取消交付：${name}。请告知用户重新建会话后重新交付。`;
				if (conv && conversationId !== bot.id) await appendRoomMsg(conv.id, {
					role: "bot",
					botId: bot.id,
